...
import { useChangesLegislation } from "../../../../store";
import dayjs from "dayjs";

import "../create-form-legislation/styles/form-create-body.css";
...
type UpdateFormLegislationProps = {
  onSubmit: (value: Schema) => void;
  onClose: () => void;
};

export const UpdateFormLegislation: FC<UpdateFormLegislationProps> = ({
  onSubmit,
  onClose,
}) => {
  const {
    currentEditLegislation,
    actions: { getLegislation },
  } = useChangesLegislation((state) => state);

  const methods = useForm({
    defaultValues: getLegislation(currentEditLegislation),
    resolver: zodResolver(updateLegislationSchema),
    mode: "all",
  });
  ...
  return (
    <AntdForm layout="vertical" onSubmitCapture={methods.handleSubmit(submit)}>
      <div>Редактирование изменения в законадательства</div>
